...
Resolution order is an explicit CLI source, an explicit CLI profile, the
selected or nearest `sumi-docs.config.json` source/profile, then the trusted
project root `docs/`. `--config` selects which config file is read. In a
directory without Git, discovery does not walk upward.
...
### Explicit corpus profiles

Profiles are opt-in aliases for a host-supplied corpus. Pass `--profile
<name>`, or set `"profile": "<name>"` in `sumi-docs.config.json`; the server
then reads `SUMI_DOCS_PROFILE_<NAME>_SOURCE` from its process environment. The
name is lower-case alphanumeric or underscore text and is converted to upper
case for the environment key. The value is an absolute local Markdown/MDX
directory or an HTTPS manifest URL. Sumi Docs does not ship a profile registry
or a machine-specific path.

For example, a harness can start the read-only stdio server with:
...
The profile source is selected only when the profile is named. With no profile,
the normal project `docs/` discovery remains unchanged. A positional source
and `--profile` cannot be combined. The profile environment value is external
host input, so the same product can serve different corpora without embedding
their paths.
